...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    address override public erc20TokenAddress; 
    ...
    function openTrove(uint _maxFeePercentage, uint _LUSDAmount, uint _ERC20Amount, address _upperHint, address _lowerHint) external override {
        ...
        vars.ICR = LiquityMath._computeCR(_ERC20Amount, vars.compositeDebt, vars.price);
        vars.NICR = LiquityMath._computeNominalCR(_ERC20Amount, vars.compositeDebt);

        if (isRecoveryMode) {
            _requireICRisAboveCCR(vars.ICR);
        } else {
            _requireICRisAboveMCR(vars.ICR);
            uint newTCR = _getNewTCRFromTroveChange(_ERC20Amount, true, vars.compositeDebt, true, vars.price);  // bools: coll increase, debt increase
            _requireNewTCRisAboveCCR(newTCR); 
        }

        // Set the trove struct's properties
        contractsCache.troveManager.setTroveStatus(msg.sender, 1);
        contractsCache.troveManager.increaseTroveColl(msg.sender, _ERC20Amount);
        ...
        emit TroveUpdated(msg.sender, vars.compositeDebt, _ERC20Amount, vars.stake, BorrowerOperation.openTrove);
        emit LUSDBorrowingFeePaid(msg.sender, vars.LUSDFee);
    }

    // Send ETH as collateral to a trove
    function addColl(uint _ERC20Amount, address _upperHint, address _lowerHint) external override {
        _adjustTrove(msg.sender, 0, _ERC20Amount, 0, false, _upperHint, _lowerHint, 0);
    }

    // Send ETH as collateral to a trove. Called by only the Stability Pool.
    function moveERC20GainToTrove(uint _ERC20Amount, address _borrower, address _upperHint, address _lowerHint) external override {
        _requireCallerIsStabilityPool();
        _adjustTrove(_borrower, 0, _ERC20Amount, 0, false, _upperHint, _lowerHint, 0);
    }
    ...
    function adjustTrove(uint _maxFeePercentage, uint _collWithdrawal, uint _collDeposit, uint _LUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint) external override {
        _adjustTrove(msg.sender, _collWithdrawal, _collDeposit, _LUSDChange, _isDebtIncrease, _upperHint, _lowerHint, _maxFeePercentage);
    }

    /*
    * _adjustTrove(): Alongside a debt change, this function can perform either a collateral top-up or a collateral withdrawal. 
    * It therefore expects either a _collDeposit, or a _collWithdrawal argument.
    * If both are positive, it will revert.
    */
    function _adjustTrove(address _borrower, uint _collWithdrawal, uint _collDeposit, uint _LUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        ContractsCache memory contractsCache = ContractsCache(troveManager, activePool, lusdToken);
        ...
        _requireNonZeroAdjustment(_collWithdrawal, _collDeposit, _LUSDChange);
        ...
        // Get the collChange based on whether or not ERC20 Collateral was sent in the transaction
    ...
    function claimCollateral() external override {
        // send ERC20 Collateral from CollSurplus Pool to owner
    ...
    function _requireNonZeroAdjustment(uint _collWithdrawal, uint _collDeposit, uint _LUSDChange) internal view {
